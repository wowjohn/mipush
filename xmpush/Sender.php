...
class Sender extends HttpBase
{
    ...
    //指定regId列表群发
    public function sendToIds(Message $message, Array $regIdList, $retries = 1)
    {
        $fields      = $message->getFields();
        $jointRegIds = '';
    ...
    public function multiSendAtTime($targetMessages, $type, $timeToSend, $retries = 1)
    {
        ...
        $fields = ['messages' => json_encode($data), 'time_to_send' => $timeToSend];
    ...
    //指定userAccount列表群发
    public function sendToUserAccounts(Message $message, Array $userAccountList, $retries = 1)
    {
        $fields            = $message->getFields();
        $jointUserAccounts = '';
    ...
    public function multiTopicBroadcast(Message $message, Array $topicList, $topicOp, $retries = 1)
    {
